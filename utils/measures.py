...
def stdev(individual, labels_pred, num_clusters, points):
	...
	for i in range(num_clusters):
		index_list = np.where(labels_pred == i)
		distances = np.append(distances, np.linalg.norm(points[index_list] - startpts[i], axis=1))
...
def SSE(individual, labels_pred, num_clusters, points):
	...
	fitnessValues = np.linalg.norm(points - centroids_for_points, axis=1) ** 2
...
def purity(labels_true, labels_pred):
	...
	k = (np.max(labels_true) + 1).astype(int)
...
def entropy(labels_true, labels_pred):
	...
	for i in range(k):
		...
		for j in range(k):
			...
			e = np.shape(z)[0]
			if (e != 0):
				entropy_i = entropy_i + (e / np.shape(t1)[1]) * np.log(e / np.shape(t1)[1])

		a = np.shape(t1)[1]
		b = np.shape(labels_true)[0]
		entropy = entropy + ((a / b) * ((-1 / np.log(k)) * entropy_i))
