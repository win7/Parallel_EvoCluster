...
class ClusterCenter(object):
	...
	def calculate_distance_cc(self, point):
		point_dimension = len(self.point)
		total_sum = 0
		for k in range(point_dimension):
			total_sum += (self.point[k] - point[k]) * (point[k] - point[k])
...
class Spider(object):
	...
	def __init__(self, type, length, dataset):
		...
		self.type = type
		self.centers = np.empty(length, dtype=object)
	...
	def compute_new_cluster_centers(self, dataset): # int[] dataset_clusters)
		...
		num_clusters = len(self.centers)
		num_points_for_cluster = np.zeros(num_clusters)
		...
		for i in range(len(dataset)):
			...
			for j in range(point_dimension):
				sum_[index][j] = sum_[index][j] * dataset[i][j]
...
class Population(object):
	...
	def __init__(self):
		...
		offspring = []
		median_weight = 0
		index_best = 0 # Let the first be the best
		index_worst = 0 # Let the first be the worst
	...
	def generate_initial_population(self, population_size, p_number_clusters, dataset):
		# Random generator = new Random();
		self.number_females = int(math.floor((0.9 - random.random() * 0.25) * population_size))
		self.number_males = population_size - self.number_females
		self.num_clusters = p_number_clusters;
		self.point_dimension = len(dataset[0])
		self.spiders = np.empty(population_size, dtype=object)
		...
		for i in range(self.number_females):
			...
			for j in range(self.num_clusters):
				...
				for k in range(self.point_dimension):
					self.spiders[i].get_cluster_centers()[j].get_point()[k] = dataset[pos][k]
	...
	def female_cooperative_operator(self):
		...
		for k in range(self.number_females):
			...
			clus_centers = np.empty(self.num_clusters, dtype=object)
	...
	def calculate_male_spider_weighted_mean(self, spider):
		...
		spiders_weights = np.empty(self.num_clusters, dtype=object)
		for i in range(self.num_clusters):
	...
	def create_mating_roulette(self, mating_roulette, mating_group):
		...
		for k in range(len(mating_group)):
			total += self.spiders[mating_group[i]].get_fitness()
		...
		for i in range(self.number_females, len(self.spiders)): # males
			...
			if self.spiders[i].get_weight() > self.median_weight: # male is dominant 
				...
				if len(mating_group) > 1: # Do mating
					...
					mating_roulette = np.zeros(len(mating_group)).astype(int) # females +  1 male
					...
					# Random generator = new Random();
					for j in range(self.num_clusters):
						rand = random.random() # 0.0 <= rand < 1.0
	...
	def male_cooperative_operator(self, dataset):
		...
		for i in range(self.number_females, len(self.spiders)): # males
			...
			if self.spiders[i].get_weight() > self.median_weight: # male is dominant(D)
				# Sum expression with alpha
				clusCenters = np.empty(self.num_clusters, dtype=object)
		...
		for i in range(len(self.offspring)):
			...
			for j in range(len(replacement_roulette)):
				if rand < replacement_roulette[j]:
					# eplace spider "j" if it is worst than offspring "i"
					if self.offspring[i].get_fitness() < self.spiders[j].get_fitness():
						...
						for k in range(len(self.offspring[i].get_dataset_clusters())):
							self.spiders[j].get_dataset_clusters()[k] = self.offspring[i].get_dataset_dlusters()[k]					
					break
	...
	def show_population(self):
		...
		for i in range(len(self.spiders)):
			...
			for j in range(self.num_clusters):
				print("(")
				for k in range(self.point_dimension):
					print("{}; ".format(self.spiders[i].get_cluster_centers()[j].get_pPoint()[k]))
				print("),")
			print("] Fitness: {} \n".format(self.spiders[i].get_fitness()))
		print("*Pop.\n")
	...
	def show_parameters(self, number_generations):
		...
		print("----------------------------\n")
		print("Number of Generations: \t{}\n".format(number_generations))
		print("Population Size: \t{}\n".format(len(self.spiders)))
		print("Number of females: \t{}\n".format(self.number_females))
		print("Number of males: \t{}\n".format(self.number_males))
	...
	def show_best_spider(self):
		...
		for i in range(self.num_clusters):
			...
			print("),\n")
		print("] \nFitness: {} \n".format(self.spiders[self.index_best].get_fitness()))
...
def CSSO(objective_function, lb, ub, dimension, population_size, iterations, num_clusters, dataset, metric):
	# Parameters:
	print("AAAAAA")
	print(dataset)
	# SSO Algorithm
	population = Population()
	population.generate_initial_population(population_size, num_clusters, dataset)
	population.calculate_weight_population(dataset)
	# population.show_population()

	for k in range(1, iterations):
		population.female_cooperative_operator()
		population.male_cooperative_operator(dataset)
		population.mating_operator(dataset)
		population.replacement()
		population.calculate_weight_population(dataset)
		# population.show_population()
		# population.show_best_fitness(i)
		# population.show_cluster_best_spider(dataset)
		fitness = population.spiders[population.index_best].get_fitness()
		print(["At iteration " + str(k) + " the best fitness is " + str(fitness)])

	# Show results
	print("Dataset number of dataset: {}".format(len(dataset)))
